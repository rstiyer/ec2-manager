...
class State {
  ...
  async reportEbsUsage({region, volumetype, state, totalcount, totalgb}, client) {
    assert(typeof region === 'string');//, 'wrong type for region');
    assert(typeof volumetype === 'string');//, 'wrong type for volumetype');
    assert(typeof state === 'string');//, 'wrong type for state');
    assert(typeof totalcount === 'number');//, 'wrong type for totalcount');
    assert(typeof totalgb === 'number');//, 'wrong type for totalgb');

    // We only want to save the most recent data for each region, volume type, and state
    // tuple, so should update the totalcount, totalgb, and touched column in the event
    // of a conflict.
    let text = [
      'INSERT INTO ebsusage (region, volumetype, state, totalcount, totalgb)',
      'VALUES ($1, $2, $3, $4, $5)',
      'ON CONFLICT (region, volumetype, state)',
      'DO UPDATE SET totalcount = EXCLUDED.totalcount, totalgb = EXCLUDED.totalgb, touched = now();',
    ].join(' ');
    let values = [region, volumetype, state, totalcount, totalgb];
    let name = 'report-ebs-usage';
    ...
    assert(result.rowCount === 1, 'inserting EBS usage had incorrect rowCount');
  }

  //async removeStaleEbsUsageData({region, type, state}, client) {
  //  let text = 'DELETE from ebsusage WHERE region = $1 AND type = $2 AND state = $3';
  //  let values = [region, type, state]
  //}
