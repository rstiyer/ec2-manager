...
class HouseKeeper {
  ...
  async _submitVolumeTotals(totalsByCategory) {
    // Assumption:
    ...
    await Promise.all(Object.keys(totalsByCategory).map(async region => {
        
      let volumeTypes = totalsByCategory[region];
      assert(typeof volumeTypes === 'object');

      await Promise.all(Object.keys(volumeTypes).map(async type => {
        ...
        volumesMonitor.measure('${region}.type:${type}.unused-gb-usage', unused.gb);
        volumesMonitor.measure('${region}.type:${type}.unused-count', unused.count);
        volumesMonitor.measure('${region}.type:${type}.active-gb-usage', unused.gb);
        volumesMonitor.measure('${region}.type:${type}.active-count', unused.count);
        ...
        try {
          await this.state.reportEbsUsage({
            region,
            volumetype: type,
            state: 'active',
          ...
          log.error(err, 'Error reporting EBS usage');
        }
      }));
    }));
  ...
  async _handleVolumeData(volumes) {
    ...
    await this._submitVolumeTotals(totals);
  }
  ...
  async sweep() {
    ...
    await this._handleVolumeData(volumesByRegion);
